...
export default function KineticGrid({
  ...
  useEffect(() => {
    ...
    // Listen on window so grid reacts anywhere — host has pointer-events:none
    ...
    const frame = () => {
      ...
      for (const d of dots) {
        const prox = m.active
          ? Math.max(0, 1 - Math.sqrt((m.x - d.x) ** 2 + (m.y - d.y) ** 2) / R)
          : 0
        ctx.globalAlpha = 0.15 + prox * 0.85
        ctx.fillStyle = dotColor
        ctx.beginPath()
        ctx.arc(d.x, d.y, 0.7 + prox * 2.0, 0, 2 * Math.PI)
        ctx.fill()
      }
      ...
      if (trail) {
        ...
        for (let i = 1; i < tr.length; i++) {
          ...
          const age = now - b.t
          if (age > 300) continue
          ctx.globalAlpha = Math.max(0, 1 - age / 300) * 0.75
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: "hidden",
        background,
        pointerEvents: "none",  // clicks pass through to content above
        zIndex: 0,
        ...(style || {}),
      }}
    >
